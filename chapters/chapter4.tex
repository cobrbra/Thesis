...
%     \[\{Y^{(0)}, Y^{(1)}\} \perp\!\!\!\perp A | X. \]
% \end{enumerate}
% These are considered sufficient to estimate the \emph{\gls{ate}}. Note that in the case of a \glsfirst{rct}, all the above apply except that the propensity function $\pi(x)$ is known (often constant) and unconfounded. In this case we get the two conditions above for free.
